<template>
  <div class="flex flex-1 flex-row w-full overflow-x-hidden overflow-y-auto relative">
    <TrackListIcon :listData="showTrackList" :offsetTop="startY" />
    <div
        class="ascrollbar flex-1 overflow-x-auto overflow-y-auto flex-col shrink-0 grow relative"
        ref="trackList"
        @scroll="handleScroll"
        @wheel="handleWheel"
    ...
    >
      ...
      >
        <template v-if="showTrackList.length === 0">
          <div class="flex justify-center items-center h-24 m-auto w-2/3 rounded-md text-sm  dark:text-gray-500 text-gray-400">
            <!-- <VideoIcon class="text-xl mr-4" /> -->
            {{ $t("TrackContainer.TrackList.Note") }}
          </div>
        </template>
        <div
            v-else
            ...
            id="track-container"
        >
          <template v-for="(lineData, lineIndex) of showTrackList" :key="lineData.list.reduce((r, item) => r + item.id, lineData.controller_key)">
            <TrackLine
              :style="{
        ...
        <TrackPreviewPoint v-show="showTrackList.length !== 0" /> 
        <template v-if="showTrackList.length !== 0">
          <div
...
<script setup lang="ts">
  ...
  import TrackPreviewPoint from '@/components/item/trackItem/TrackPreviewPoint.vue';
  ...
  import { Track, TrackLineItf } from '@/class/Track';
  ...
  const showTrackList = shallowRef([]);

  watchEffect(() => {
    const result = store.trackList.map((line) => {
  ...
  function handlerSelectFrame(frame: number) {
    ...
    const startFrame = playFrame < 0 ? 0 : playFrame > store.frameCount ? store.frameCount : playFrame;
    previewStore.isPause = true;
    previewStore.jumpTimelineTo(startFrame);
  }
  let maxDelta = 0;
  ...
  function isOverlap(dragItem, line, { start, end }: { start: number, end: number }) {
    ...
    // 处理边界问题
    if (nodes[0].start >= end-start && nodes[0].start >= end) {
      return { overlap: false, index: 0 };
    }
    ...
    for (let i = 1; i <= nodes.length - 1; i++) {
      if(nodes[i].start - nodes[i-1].end >= end - start && start >= nodes[i-1].end && end <= nodes[i].start){
        return { overlap: false, index: i };
      }
    }
    // 当重叠时，创建新行，所以插入位置为0
    return { overlap: true, index: 0 };
  ...
  function getInsertInfo(): InsertInfo {
    ...
    //fix bug that the left and right fixline at the same time
    let start = 0;
    let end = 0;
    if(fixPosition.right){//note: must judge fixPosition.right or fixPosition.end, because fixPosition.left and fixPosition.start could be 0.
      end = fixPosition.end || getSelectFrame(right, store.trackScale, 30);
      start = end + dragItem.start - dragItem.end;
    }else{
      start = fixPosition.start || getSelectFrame(left, store.trackScale, 30);
      end = start - dragItem.start + dragItem.end;
    }
  ...
  function getFixLine(x: number, distance = 10) {
    ...
    const result: { position: number; frame: number; }[] = [];
    ...
    const trackPlayPointX = getGridPixel(store.trackScale, previewStore.timelineTS);
    if (Math.abs(trackPlayPointX - x) <= distance) {
      result.push({ position: trackPlayPointX, frame: previewStore.timelineTS });
    }
  ...
  async function insert(insertInfo: InsertInfo) {
    ...
    const [deleteLineIndex, deleteItemIndex] = store.getClipIndexById(dragInfo.id)

    // remove but not delete the clip from this track.
    store.trackList[deleteLineIndex].list.splice(deleteItemIndex, 1);

    if (insertInfo.isNewLine) {
      // 插入新行
      const track_key = await store.addTrack(newTrackItem.type);

      store.ClipMoved(newTrackItem as Track,  store.trackList[deleteLineIndex].controller_key, track_key);
      previewStore.needSeek = 'movedClips'
      store.needSeekClips.add(newTrackItem);

      const track_idx = store.trackList.length - 1;//store.findTrackIndexByKey(track_key)
      store.trackList[track_idx].list.push(newTrackItem as Track);

      //move to the next position
      const tk = store.trackList[track_idx];
      store.trackList.splice(track_idx, 1);
      store.trackList.splice(insertInfo.insertIndex, 0, tk);
    } else {
      // 插入当前行
      store.trackList[insertInfo.insertIndex].list.splice(insertInfo.itemIndex, 0, newTrackItem);

      store.ClipMoved(newTrackItem as Track,  store.trackList[deleteLineIndex].controller_key, store.trackList[insertInfo.insertIndex].controller_key);
      previewStore.needSeek = 'movedClips';
      store.needSeekClips.add(newTrackItem);
    }
    ...
    if (deleteIndex !== -1) {
      await store.removeTrackLine(deleteIndex);
    }
  }
  ...
  async function onMouseUp(event: MouseEvent) {
    if (dragElement) {
      ...
      // console.log("store.dragData.moveX:", store.dragData.moveX);
      // console.log("store.dragData.moveY:", store.dragData.moveY);
      if(store.dragData.moveX || store.dragData.moveY){//only click the clip.
        const left = store.dragData.moveX + curCoord.left;
        const right = store.dragData.moveX + curCoord.right;
